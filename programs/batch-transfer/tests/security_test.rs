...
/// 测试整数溢出防护
/// 
/// 这个测试验证智能合约是否能够正确防止整数溢出攻击：
/// 1. 测试超大金额转账 - 使用u64::MAX作为转账金额，预期因为金额超过发送者余额而失败
/// 2. 测试手续费计算溢出 - 使用一个特殊金额，可能导致fee * amount计算时溢出，预期合约能检测并拒绝
///
/// 预期结果：
/// - 两次测试都应该返回适当的错误码，表明合约能够防止整数溢出攻击
#[test]
...
/// 测试重入攻击防护
/// 
/// 这个测试验证智能合约是否能够正确防止重入攻击：
/// 1. 模拟一个常规的SOL转账操作
/// 2. 假设在转账过程中，接收方尝试在回调中重新调用合约的转账功能（重入攻击）
/// 3. 测试合约是否能够识别并拒绝这种重入请求
///
