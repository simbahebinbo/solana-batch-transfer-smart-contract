...
async fn test_insufficient_balance() {
    ...
    create_token_account(&mut banks_client, &sender_token, &mint.pubkey(), &sender.pubkey(), recent_blockhash).await;
    create_token_account(&mut banks_client, &recipient1_token, &mint.pubkey(), &recipient1.pubkey(), recent_blockhash).await;
    create_token_account(&mut banks_client, &recipient2_token, &mint.pubkey(), &recipient2.pubkey(), recent_blockhash).await;

    // 铸造少量代币给发送者
    mint_tokens(&mut banks_client, &mint, &sender_token.pubkey(), &admin, 1_000_000, recent_blockhash).await;
    ...
    // 验证代币余额未变化
    let sender_token_account = banks_client
        .get_account(sender_token.pubkey())
        .await
        .unwrap()
        .unwrap();
    let sender_token_account = TokenAccount::unpack(&sender_token_account.data).unwrap();
    assert_eq!(sender_token_account.amount, 1_000_000);

    // 验证接收者未收到代币
    let recipient1_token_account = banks_client
        .get_account(recipient1_token.pubkey())
        .await
        .unwrap()
        .unwrap();
    let recipient1_token_account = TokenAccount::unpack(&recipient1_token_account.data).unwrap();
    assert_eq!(recipient1_token_account.amount, 0);

    let recipient2_token_account = banks_client
        .get_account(recipient2_token.pubkey())
        .await
        .unwrap()
        .unwrap();
    let recipient2_token_account = TokenAccount::unpack(&recipient2_token_account.data).unwrap();
    assert_eq!(recipient2_token_account.amount, 0);
} 