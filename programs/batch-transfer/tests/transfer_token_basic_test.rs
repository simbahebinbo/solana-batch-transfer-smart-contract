...
async fn test_batch_transfer_token() {
    ...
    let transfers = vec![
        ...
        (recipient2_token.pubkey(), 2_000_000),
    ];
    ...
    for account in accounts.iter_mut() {
        if account.pubkey == sender.pubkey() {
            account.is_signer = true;
            account.is_writable = true;
        }
        if account.pubkey == sender_token.pubkey() {
            account.is_writable = true;
        }
    }
    ...
    let sender_token_account = banks_client
        ...
        .unwrap()
        .unwrap();
    let sender_token_state = TokenAccount::unpack(&sender_token_account.data).unwrap();
    assert_eq!(sender_token_state.amount, 97_000_000); // 100_000_000 - 1_000_000 - 2_000_000
    ...
    let recipient1_token_account = banks_client
        ...
        .unwrap()
        .unwrap();
    let recipient1_token_state = TokenAccount::unpack(&recipient1_token_account.data).unwrap();
    assert_eq!(recipient1_token_state.amount, 1_000_000);
    ...
    let recipient2_token_account = banks_client
        ...
        .unwrap()
        .unwrap();
    let recipient2_token_state = TokenAccount::unpack(&recipient2_token_account.data).unwrap();
    assert_eq!(recipient2_token_state.amount, 2_000_000);
} 