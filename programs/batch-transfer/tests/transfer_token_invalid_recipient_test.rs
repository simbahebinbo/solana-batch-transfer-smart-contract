...
async fn test_invalid_recipient() {
    ...
    // 铸造代币给发送者
    mint_tokens(&mut banks_client, &mint, &sender_token.pubkey(), &admin, 100_000_000, recent_blockhash).await;
    ...
    assert_eq!(sender_token_account.amount, 100_000_000);
    ...
    // 验证无效接收者账户不存在
    let invalid_recipient_account = banks_client.get_account(invalid_recipient).await.unwrap();
    assert!(invalid_recipient_account.is_none());
} 